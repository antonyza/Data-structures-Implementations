...
#define RECORDS_NUM 20000 // Number of random records to insert & search
					...
					char* file_name)
{
  ...
  for (int i = 0; i < RECORDS_NUM; i++) {
	...
	bplus_record_find(file_desc, info, record_get_key(&schema, &record), &result);
	printf("Verifying that inserted records exists: ");
	if (result != NULL) {
	  printf("Found!\n");
	  free(result);
	} else {
